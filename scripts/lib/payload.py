...
def build_macro(df: pd.DataFrame) -> dict:
    fallback_date = datetime.today().strftime("%Y-%m-%d")
    if df.empty:
        return {
            "selic": None, "cdi": None, "ipca": None,
            "selic_proj": None, "ipca_proj": None,
            "cenario": "indisponivel", "descricao": "Sem dados macro disponíveis.",
            "data_ref": fallback_date,
            "inadimplencia_pj": None, "inadimplencia_pf": None,
            "ibc_br": None, "dolar_venda": None,
        }
    last = df.iloc[-1]
    ...
    ipca_serie = pd.to_numeric(df.get("ipca_mensal"), errors="coerce").dropna().tail(12)
...
# ─── FIDCs ───────────────────────────────────────────────────────────
def _fidc_detalhe_row(r: pd.Series) -> dict:
    return {
...
def _fidc_resumo_row(r: pd.Series) -> dict:
    return {
        "cnpj":                cnpj_fmt(r.get("CNPJ")),
        "fundo":               to_str(r.get("FUNDO")),
        "score_risco":         to_float(r.get("SCORE_RISCO"), 0.0, 1),
...
def _scatter_row(r: pd.Series) -> dict:
    return {
...
def build_fidcs(geral: pd.DataFrame, resumo: pd.DataFrame) -> dict:
    if geral.empty:
        return {"stats": {"total_classes": 0, "total_fundos": 0, "distribuicao": {"por_risco": {}, "por_perfil": {}, "por_cota": {}}},
                "resumo": [], "detalhe": [], "scatter": []}
    ...
    deduped = confiaveis.drop_duplicates(subset=["CNPJ", "FUNDO", "TIPO_COTA"]) \
    ...
    if not resumo.empty:
        ...
        resumo_deduped = resumo_filtrado.drop_duplicates(subset=["CNPJ"]) \
                                        .sort_values("SCORE_RISCO", ascending=True)
    else:
    ...
    scatter_sample = (scatter_pool.sample(min(MAX_SCATTER, len(scatter_pool)), random_state=42)
                      if len(scatter_pool) > MAX_SCATTER else scatter_pool)

    return {
        "stats": {
            "total_classes": int(len(geral)),
...
def _cliente_row(r: pd.Series) -> dict:
    return {
...
def build_clientes(df: pd.DataFrame) -> dict:
    if df.empty:
...
# ─── MATCHES ─────────────────────────────────────────────────────────
def _match_row(r: pd.Series) -> dict:
    return {
        "cpf":              mask_cpf(r.get("CPF")),
...
def _ranking_row(r: pd.Series) -> dict:
    return {
        "fundo":             to_str(r.get("FUNDO")),
        "tipo_cota":         to_str(r.get("TIPO_COTA"), "UNICA"),
...
def build_matches(todos: pd.DataFrame, ranking: pd.DataFrame) -> dict:
    return {
        "total": int(len(todos)),
...
def _credit_row(r: pd.Series) -> dict:
    return {
...
def build_credit(df: pd.DataFrame) -> dict:
    if df.empty:
