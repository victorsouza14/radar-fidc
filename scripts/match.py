#!/usr/bin/env python3
"""Engine de matching cliente × FIDC.
...
Funções:
    score_perfil:     alinhamento de perfil cliente×fundo (matriz em perfil_rules)
...
def score_risco(apetite_cliente: float, risco_fundo: float) -> float:
    ...
    if diff > 0:
        penalidade = diff * PENALIDADE_RISCO_ACIMA
    else:
        penalidade = abs(diff) * PENALIDADE_RISCO_ABAIXO
...
def calcular_match(cliente: pd.Series, fundo: pd.Series) -> MatchScores | None:
    ...
    match = (
        s_perfil    * PESO_PERFIL +
        s_risco     * PESO_RISCO +
        s_retorno   * PESO_RETORNO +
        s_hist      * PESO_HISTORICO
    )
    return MatchScores(
...
def gerar_motivo(cliente: pd.Series, fundo: pd.Series, scores: MatchScores) -> str:
    ...
    if pd.notna(inad):
        nivel = (
            "inadimplência baixa" if inad < 5 else
            "inadimplência moderada" if inad < 15 else
...
def rodar_match(
    ...
    print(f"\n  Calculando matches ({len(clientes)} clientes × {len(fundos)} fundos)...")
...
def gerar_resumo(df_matches: pd.DataFrame) -> pd.DataFrame:
    ...
    for (cpf, nome, perfil), grupo in grupos:
        ...
        linha: dict = {
            "CPF": cpf, "CLIENTE": nome, "PERFIL": perfil,
...
def exportar(df_matches: pd.DataFrame, df_resumo: pd.DataFrame, output_path: str) -> None:
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        for perfil in ("CONSERVADOR", "MODERADO", "ARROJADO"):
            ...
            if not sub.empty:
                sub.sort_values(["CLIENTE", "RANK"]).to_excel(
                    writer, sheet_name=perfil, index=False
                )
...
def main() -> int:
    ...
    for perfil in ("CONSERVADOR", "MODERADO", "ARROJADO"):
        ...
        for _, r in sub.iterrows():
            print(f"  {r['CLIENTE']:<20} → {r['FUNDO'][:45]:<45} | match {r['MATCH_SCORE']:.0f} | retorno {r['RETORNO_ANUAL']:.1f}%")
        print()
